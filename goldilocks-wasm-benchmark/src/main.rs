...
fn benchmark_goldilocks_multiplication() -> f64 {
    ...
    println!("Goldilocks multiplication: 1,000,000 ops in {:.3}ms", duration_ms);
    duration_ms
}
...
fn benchmark_goldilocks_monty_multiplication() -> f64 {
    ...
    let duration_ms = duration.as_secs_f64() * 1000.0;
    println!("Goldilocks Monty multiplication: 1,000,000 ops in {:.3}ms", duration_ms);
    duration_ms
}
...
fn benchmark_goldilocks_monty_addition() -> f64 {
    ...
    let duration_ms = duration.as_secs_f64() * 1000.0;
    println!("Goldilocks Monty addition: 1,000,000 ops in {:.3}ms", duration_ms);
    duration_ms
}
...
fn main() {
    ...
    if monty_mult_time > 0.0 {
        ...
        if mult_speedup > 1.0 {
            println!("✅ Monty multiplication is {:.2}x faster than standard", mult_speedup);
        } else {
            println!("⚠️  Standard multiplication is {:.2}x faster than Monty", 1.0 / mult_speedup);
        }
    }
    ...
    if monty_add_time > 0.0 {
        ...
        if add_speedup > 1.0 {
            println!("✅ Monty addition is {:.2}x faster than standard", add_speedup);
        } else {
            println!("⚠️  Standard addition is {:.2}x faster than Monty", 1.0 / add_speedup);
        }
    }
