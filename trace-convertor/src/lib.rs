    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ConversionError::PowerOfTwoPadding { current, required } => {
                write!(f, "Power of 2 padding error: current={}, required={}", current, required)
            },
        }
    }
}
...
impl TraceConverter {
    ...
    pub fn convert<F: PrimeField>(
        ...
        println!("Converting trace: {}×{} -> {}×{}", height, width, padded_height, width);
        ...
        for row_idx in 0..padded_height {
            for col_idx in 0..width {
                let felt_value = if row_idx < height {
...
impl TraceStats {
    pub fn print(&self) {
        ...
        println!("  Padded height: {} (2^{})", self.padded_height, self.log_height);
    ...
    fn test_power_of_two_padding() {
        ...
        for (original, expected) in original_sizes.iter().zip(expected_padded.iter()) {
            ...
            assert_eq!(padded, *expected, 
                "Original size {} should pad to {}, got {}", original, expected, padded);
            assert!(padded.is_power_of_two(), 
                "Padded size {} should be power of 2", padded);
        }
    }
}
